...
session_start();
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] != true || $_SESSION["user"] != "doctor") {
    header("location:login");
    exit;
}

$doctor_id = $_SESSION["doctor_id"];
$query = mysqli_query($con, "SELECT * FROM doctors WHERE doctor_id = '$doctor_id'");
...
<body>
    ...
    <div class="bg-primary-subtle py-2 px-4 d-flex justify-content-between align-items-center border-bottom border-3 border-dark shadow">
        <h3 class="m-0">Welcome Dr. <?php echo $row["first_name"] . " " . $row["last_name"] ?></h3>
        <div>
